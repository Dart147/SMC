...
test.describe('SMC Anti-Cheat System (防作弊系統 E2E 測試)', () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    // 2. 填入指定的測試帳號與密碼
    await page.getByPlaceholder('請輸入面試通知上的帳號').fill('USER-6370');
    await page.getByPlaceholder('請輸入密碼').fill('K1MGH2');
  ...
  test('連續 3 次觸發失焦，系統應正確顯示警告、累加次數並強制踢出', async ({ page }) => {
    ...
    page.once('dialog', async dialog => {
      expect(dialog.message()).toBe('🚨 系統偵測您已嚴重違規達 3 次！系統已自動交卷並將您強制登出。');
      await dialog.accept();
    });
