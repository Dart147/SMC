          ...
          {error ? (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="lg:col-span-2 space-y-4">
                <div className="bg-black rounded-xl border border-gray-800 overflow-hidden">
                  <div className="bg-gray-800 px-4 py-2 flex justify-between items-center">
                    <span className="text-xs font-mono text-gray-300">{data.language.toUpperCase()}</span>
                    <span className="text-[10px] bg-green-900/50 text-green-400 px-2 py-0.5 rounded border border-green-500/30">FINAL SUBMISSION</span>
                  </div>
              ...
              <div className="space-y-4">
                {/* 誠實度卡片 */}
                <div className={`p-5 rounded-xl border transition-all ${data.warningCount > 0 ? 'bg-red-900/20 border-red-500 shadow-[0_0_15px_rgba(239,68,68,0.1)]' : 'bg-green-900/20 border-green-500/50'}`}>
                  <p className="text-xs uppercase font-black text-gray-500 tracking-widest">誠實度檢查</p>
                  <div className="flex items-baseline gap-2 mt-2">
                    <p className={`text-4xl font-black ${data.warningCount > 0 ? 'text-red-500' : 'text-green-500'}`}>
                      {data.warningCount}
                    </p>
                ...
                <div className="bg-gray-800/50 p-5 rounded-xl border border-gray-700 shadow-inner">
                  <p className="text-xs uppercase font-black text-gray-500 tracking-widest">測驗評分</p>
                  <p className="text-4xl font-black text-blue-400 mt-2">{data.score} <span className="text-lg text-gray-600">/ 100</span></p>
                  <div className="mt-4 pt-4 border-t border-gray-700 space-y-2">
                    <div className="flex justify-between text-sm">
                      <span className="text-gray-500">判定狀態</span>
                      <span className={`font-bold ${data.status === 'Accepted' ? 'text-green-400' : 'text-orange-400'}`}>{data.status}</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="p-4 bg-gray-900 border border-gray-800 rounded-xl">
                    <p className="text-[10px] text-indigo-400 uppercase font-bold tracking-wider">面試者資訊</p>
