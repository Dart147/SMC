                  ...
                  <div className="flex items-center justify-between w-2/3 pr-4">
                    ...
                    <div className="flex flex-col items-center">
                      ...
                      <span className="font-bold text-gray-300">
                        {candidate.submissions.length} 題
                      </span>
                    </div>
                  ...
                  <div className="bg-[#141414] border-t border-gray-800 p-6">
                    ...
                    <div className="space-y-3">
                      {candidate.submissions.map((sub) => (
                        <div
                          key={sub.id}
                          className="grid grid-cols-6 gap-4 items-center bg-[#1e1e1e] p-4 rounded-xl border border-gray-800 hover:border-gray-600 transition-colors"
                        >
                          <div className="col-span-2 flex flex-col">
                            <span className="text-gray-200 font-bold">{sub.problemTitle}</span>
                            <span
                              className={`text-xs mt-1 font-bold ${sub.status === "Accepted" ? "text-green-400" : "text-red-400"}`}
                            >
                              {sub.status}
                            </span>
                          </div>
                          <div className="flex flex-col items-center">
                            ...
                            <span className="font-mono text-sm mt-1 text-indigo-300">
                              {sub.testCases || "0/0"}
                            </span>
                          </div>
                          <div className="flex flex-col items-center">
                            <span className="text-[10px] text-gray-500 uppercase">Run Time</span>
                            <span className="font-mono text-sm mt-1 text-emerald-400">
                              {sub.runTimeMs} ms
                            </span>
                          </div>
                          <div className="flex flex-col items-center">
                            ...
                            <span
                              className={`font-mono text-sm mt-1 font-bold ${sub.codeStyleScore >= 90 ? "text-green-400" : "text-red-400"}`}
                            >
                              {sub.codeStyleScore}
                            </span>
                          </div>
