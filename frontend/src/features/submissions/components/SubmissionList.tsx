...
export const SubmissionList: React.FC<Props> = ({ submissions, isLoading }) => {
  // 💡 狀態：記錄目前哪一筆 Submission 的 ID 被展開了 (null 代表全收合)
  const [expandedId, setExpandedId] = useState<string | null>(null);
  // 💡 狀態：記錄目前要顯示哪一筆 Submission 的分析報告 (Max 新增)
          ...
          >
            ...
            >
              <div className="flex items-center space-x-6">
                {/* 狀態文字：點擊開啟詳細報告彈窗 (融合 Max 的功能) */}
                <div
                  onClick={(e) => {
                    e.stopPropagation(); // ⚠️ 防止點擊按鈕時觸發父層的 toggleExpand
                    setReportId(submission.id);
                  }}
                  className={`font-bold w-32 ${statusColor} flex items-center gap-2 hover:underline underline-offset-4 decoration-2 group cursor-pointer`}
              ...
              <div className="flex items-center space-x-6">
                {/* 測資通過比例 */}
                {!isPending && !isError && (
                  <div className="text-sm font-mono bg-gray-100 dark:bg-gray-700 px-3 py-1 rounded-full flex items-center">
                    <span
